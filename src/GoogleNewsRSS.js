...
const SERVER_PROD = "https://trendingcontent-server.vercel.app";
const API_ROOT =
  process.env.NODE_ENV === "production" ? SERVER_PROD : "http://localhost:8080";
...
export function getNews(options) {
  ...
  return xmlFetch(`${API_ROOT}/rss/topics/${path}`, { cache: "no-store" }).then(
    /** @param {document} data */ (data) => {
      const items = Array.from(data.getElementsByTagName("item")).map(
        (itemEl) => {
          const titleEl = itemEl.getElementsByTagName("title")[0];
          let title = titleEl
            ? decodeHtml(titleEl.textContent || titleEl.innerHTML)
            : "";
          ...
          const sourceName = sourceNameEl
            ? sourceNameEl.textContent || sourceNameEl.innerHTML
            : "";
          ...
          const publishedAt = dateEl
            ? new Date(dateEl.textContent || dateEl.innerHTML).toISOString()
            : "";

          const imageEl = itemEl.getElementsByTagName("media:content")[0];
          let imageURL = imageEl
            ? imageEl.attributes.getNamedItem("url").textContent
            : "";
          ...
          if (descEl) {
            ...
            sources = Array.from(descDoc.getElementsByTagName("li"))
              .map((liEl) => {
                ...
                const url = aEl
                  ? aEl.attributes.getNamedItem("href").textContent
                  : "";
