  ...
  async function selectPlan(priceID, index) {
    if (isUserSignedIn) {
      ...
      try {
        ...
        toast.error("an error accurred while getting payment link");
        console.error(error);
        ...
        {pricingPlansData.map(({ title, price, benefits, onSelect }, index) => {
          return (
            ...
            >
              ...
              <ul
                className={`${userIsSubscribed == true ? "mt-4" : ""}
              mb-5`}
              >
                {benefits.map((b) => {
                  return (
              ...
              >
                ...
                {planLoadingIndex == index ? (
                  <SpinnerLoader
                    className={`w-5 aspect-square animate-spin text-gray-700 fill-black`}
                  />
                ) : (
