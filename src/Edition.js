    ...
    cats.forEach(category => {
      getNews({ category, edition }).then(data => this.setState(oldState => {
        ...
        if (err === "CORS Error" && !this.embarrassed) {
          ...
          alert("Well this is embarrassing.\n\nOur servers don't exactly play nicely with this. More accurately they just don't consider CORS which would let us load the news directly. Instead I need to proxy the requests through the NewsEngine servers and we don't have budget to implement the proxying properly.\n\nMy advice is to try a different news edition in the options.");
        }
        console.log(err);
      });
    });
