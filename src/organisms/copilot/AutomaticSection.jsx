...
export default function AutomaticSection() {
	...
	return (
		<div className={`${!isAutomaticMode ? 'hidden' : ''} w-[min(48rem,90%)] mx-auto`}>
			<ul className='flex flex-col items-center gap-9 mx-auto mb-9 w-full'>
				{automaticSections.map(({ title, clickHandle }) => {
					return (
						<li key={title} className='mx-auto w-full overflow-hidden'>
							<div className='w-full'>
								<div className='items-center grid grid-cols-3 mb-4 uppercase'>
									<h2>{title}</h2>
									<CountdownTimer generateHandle={clickHandle} segmentName={title} />
									<button
										onClick={clickHandle}
										className='flex justify-center items-center gap-2 px-9 py-3 border border-blue-700 rounded-md text-blue-200 hover:text-blue-700 uppercase whitespace-nowrap'>
										<Image src={micIcon} alt='mic logo' width={15} height={15} />
										GENERATE {title}
										<SpinnerLoader className={segmentsData[title].isLoadingAudioUrl ? '' : 'hidden'} />
									</button>
								</div>
								<AudioPlayer src={segmentsData[title].audioUrl} />
								<div className={`${segmentsData[title].audioUrl ? '' : 'hidden'} flex items-center gap-2 mt-3`}>
									{exportUploadBtnsData.map(({ btnText, clickHandle, icon }) => {
										return (
											<button
												key={btnText}
												onClick={() => clickHandle(title)}
												className='flex justify-center items-center gap-2 px-9 py-3 border border-blue-700 rounded-md w-full uppercase'>
												<Image src={icon} alt='mic logo' width={15} height={15} />
												{btnText}
												<SpinnerLoader
													className={
														(btnText == 'export' && segmentsData[title].isLoadingDownloadAudio) ||
														(btnText == 'upload to cloud' && segmentsData[title].isLoadingExportAudio)
															? ''
															: 'hidden'
													}
												/>
