...
export default async function getNewsSegmentScript(segmentName, requestData) {
	...
	// Получаем правильный код страны для API news/weather/travel
	const countryCode = getApiCountryCode(selectedRegion)
	// Получаем соответствующий язык для выбранной страны
	const language = getLanguageForCountry(countryCode)

	console.log(`Region: ${selectedRegion}, News API country code: ${countryCode}, Language: ${language}`)
...
export function fetchNews() {
	...
	// Передаем данные о стране и языке
	const requestData = {
		location_code: countryCode,
		language: language,
	}

	getNewsSegmentScript('news', requestData)
	getNewsSegmentScript('weather', requestData)
	getNewsSegmentScript('travel', requestData)
}
